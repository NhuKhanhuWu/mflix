...
exports.testFixId = async (req, res) => {
  try {
    // update user id for comment
    const comments = Comment.find().limit(10);

    comments.forEach((comment) => {
      // Step 3: Find the user based on the email
      const user = db.users.findOne({ email: comment.email });

      if (user) {
        // Step 4: Update the comment with the user_id
        db.comments.updateOne(
          { _id: comment._id },
          {
            ...
            $unset: { email: "", name: "" }, // Optionally remove email and name fields
          }
        );
      } else {
        print(`No user found for email: ${comment.email}`);
      }
    });
  } catch (err) {
    res.status(400).json({
